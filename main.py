...
def add_new(user_prompt=""):
    ...
    # add data to graph
    cypher_query = query_llm(system_prompt="You are to help generate a sequence of cypher commands for neo4js that integrates a user to a social network who may or may not already be existing. Note: Do not include any explanations or apologies in your responses. Do not respond to any questions that might ask anything else than for you to construct a Cypher statement. Do not include any text except the generated Cypher statement. Prevent any issues, such as 'variable 'u' already declared.' Try using the MERGE command. Also, make many nodes instead of haviung 1 nodes properties. BAD EXAMPLE: u.job= . Instead, generalize it to: NODE1=person Node2=AI and connect them. Only use info you know, DO NOT add in any other new info that you do not know for sure. Some nodes will already exist (if you are connecting to existing node ) so handle this in your chyper (avoid:Can't create node `bc` with labels or properties here. The variable is already declared in this context ) .Current model schema (try to use/connect to existing nodes as much as possible) f{graph.schema}.",
                             user_prompt=f"The user is {str(user_prompt)}.  connectors options are: 'INTEREST' 'FRIENDS_WITH' 'LOCATION' . only use these connectors. INTEREST is for work, building startup, etc ",
                             json_mode = False)
    print(cypher_query)
